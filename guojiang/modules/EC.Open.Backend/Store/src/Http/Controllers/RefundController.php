...
use GuoJiangClub\EC\Open\Backend\Store\Service\RefundService;
...
class RefundController extends Controller
{
	...
	public function __construct(RefundRepository $refundRepository, RefundService $refundService, PaymentService $paymentService)
	{
	...
	public function paid()
	{
		...
		if ($channel == 'wechat') {
			...
			$merchantPay = MerchantPay::where('origin_type', 'REFUND')->where('origin_id', $refund->id)->where('channel', 'wechat')->where('channel_id', $refundAmount->id)->where('status', 'SUCCESS')->first();
			if ($merchantPay) {
