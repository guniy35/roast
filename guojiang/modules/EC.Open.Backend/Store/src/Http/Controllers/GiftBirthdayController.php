...
class GiftBirthdayController extends Controller
{
    ...
    public function index()
    {
        ...
        $lists = $this->giftActivityRepository->giftAll('gift_birthday');
        ...
        return LaravelAdmin::content(function (Content $content) use ($lists, $coupons) {
            ...
            $content->breadcrumb(
                ['text' => '生日礼', 'url' => 'store/promotion/gift/birthday', 'no-pjax' => 1],
                ['text' => '生日礼列表', 'url' => '', 'no-pjax' => 1, 'left-menu-active' => '生日礼']

            );

            $content->body(view('store-backend::gift_birthday.index', compact('lists', 'coupons')));
        });
    ...
    public function user()
    {
        ...
        $lists = $this->cardRepository->getInstantBirthdayUserByDay($where, $day);

        return LaravelAdmin::content(function (Content $content) use ($lists, $day) {

            $content->header('即将过生日用户');

            $content->breadcrumb(
                ['text' => '生日礼', 'url' => 'store/promotion/gift/birthday', 'no-pjax' => 1],
                ['text' => '即将过生日用户', 'url' => '', 'no-pjax' => 1, 'left-menu-active' => '生日礼']

            );

            $content->body(view('store-backend::gift_birthday.user', compact('lists', 'day')));
        });
    ...
    public function create()
    {
        $s_time = '';
        $e_time = '';
        if ($list = $this->giftActivityRepository->giftListEffective('gift_birthday')) {
        ...
        return LaravelAdmin::content(function (Content $content) use ($s_time, $e_time) {
            ...
            $content->header('创建生日礼');

            $content->breadcrumb(
                ['text' => '生日礼', 'url' => 'store/promotion/gift/birthday', 'no-pjax' => 1],
                ['text' => '创建生日礼', 'url' => '', 'no-pjax' => 1, 'left-menu-active' => '生日礼']

            );

            $content->body(view('store-backend::gift_birthday.create', compact('s_time', 'e_time')));
        });
    ...
    {
        $gift_new_user = $this->giftActivityRepository->getListByID($id, 'gift_birthday');
        $coupon = [];
        ...
        return LaravelAdmin::content(function (Content $content) use ($gift_new_user, $coupon) {
            ...
            $content->header('编辑生日礼');

            $content->breadcrumb(
                ['text' => '生日礼', 'url' => 'store/promotion/gift/birthday', 'no-pjax' => 1],
                ['text' => '编辑生日礼', 'url' => '', 'no-pjax' => 1, 'left-menu-active' => '生日礼']

            );

            $content->body(view('store-backend::gift_birthday.edit', compact('gift_new_user', 'coupon')));
        });
    ...
    public function update($id)
    {
        ...
        try {
            ...
            if ($input['open_coupon'] && count($coupon_id) > 0) {
                $this->giftCouponRepository->deleteWhere(['type_id' => $id, 'type' => 'gift_birthday']);
    ...
    public function store()
    {
        ...
        $input['point'] = !empty($input['point']) ? $input['point'] : 0;
        if ($list = $this->giftActivityRepository->giftListEffective('gift_birthday') && !$this->giftActivityRepository->checkAllowCreateGiftBirthday($input['starts_at'])) {
            return $this->ajaxJson(false, [], 400, '');
        };
        try {
            DB::beginTransaction();
    ...
    public function toggleStatus()
    {
        ...
        if ($status && !$this->giftActivityRepository->checkAllowCreateGiftBirthday($date)) {
