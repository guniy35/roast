...
class MicroPageController extends Controller
{
	...
	public function edit($id)
	{
		...
		return LaravelAdmin::content(function (Content $content) use ($page, $id, $adverts) {
			...
			$config = config('ibrand.advert.meta');
			...
			$micro_page_componet_article = $this->advert->where('type', 'like', '%' . 'micro_page_componet_article' . '%')->where('status', 1)->get();
			//分类商品
			...
			//猜你喜欢
			$micro_page_componet_guess_like = $this->advert->where('type', 'micro_page_componet_guess_like')->where('status', 1)->get();
			//商品分组
			...
			$content->body(view('store-backend::micro_page.page.edit', compact('page', 'id', 'adverts', 'action', 'micro_page_componet_slide', 'micro_page_componet_nav', 'micro_page_componet_cube', 'micro_page_componet_seckill', 'micro_page_componet_free_event', 'micro_page_componet_groupon', 'micro_page_componet_suit', 'micro_page_componet_goods_group', 'micro_page_componet_category', 'micro_page_componet_coupon', 'config', 'micro_page_componet_article', 'micro_page_componet_guess_like')));
		});
	}
	...
	public function updateMicroPageAd($id)
	{
		...
		try {
			...
			if (isset($input['meta'])) {
				...
				$micro_page = MicroPage::find($id);

				$micro_page->meta = json_encode($meta);
