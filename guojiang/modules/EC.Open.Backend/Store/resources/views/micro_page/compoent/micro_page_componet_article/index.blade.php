        ...
        <div id="tab-1" class="tab-pane active">
            <div class="panel-body">
                ...
                <div class="row" style="margin-bottom:20px;">
                    ...
                    <div class="col-sm-4">
                        <a target="_blank" href="{{route('admin.setting.micro.page.compoent.create',['type'=>$type,'header'=>'文章'])}}" class="btn btn-w-m btn-info">创建文章模块</a>
                    </div>
                ...
                <table class="table table-striped table-bordered table-hover">
                    ...
                    <tr>
                        ...
                        <th class="col-sm-3">文章</th>
                        <th class="col-sm-3">排列样式</th>
                    ...
                    @if($lists->count())
                        @foreach($lists as $item)
                            <tr>
                                ...
                                <td>
                                    @foreach($item->item as $key=> $citem)
                                        ...
                                        @if($citem->associate)
                                            ...
                                            <div>
                                                文章{{$key+1}}:
                                                <a href="{{route('admin.bai.jia.article.edit',$citem->associate->id)}}" target="_blank">
                                                    {{$citem->associate->title}}
                                                </a>
                                                @if($citem->associate->status!=1)(已下架)@endif
                                            </div>
                                ...
                                <td>
                                    @if($item->type=='micro_page_componet_article_|')
                                        竖排
                                    @elseif($item->type=='micro_page_componet_article_-')
                                        横排
                                    @endif
                                ...
                                <td>
                                        ...
                                        <a
                                                ...
                                                href="{{route('admin.setting.micro.page.compoent.edit',['type'=>$type,'code'=>$item->code,'header'=>'文章','model'=>$item->type])}}">
