...
@section('body')
    ...
    <div class="row">
        ...
        <form class="form-horizontal"  action="{{route('admin.setting.micro.page.compoent.getSeckillsData')}}" method="get" id="search_spu_from">
                ...
                @else
                    @if(request('seckill_id'))
                        <div class="form-group">
                            <label for="exampleInputEmail1" class="col-sm-3 control-label"></label>
                            <div class="col-sm-6">
                                <span class="label label-default">seckill_id：{{request('seckill_id')}};不存在</span>
                            </div>
                        </div>
                    @endif
        ...
        <div class="panel-body">
            ...
            <div class="table-responsive col-sm-9" id="pagesList" data-seckill_id="{{request('seckill_id')}}">
...
@section('footer')
    ...
    <button type="button" onclick="sendIds();"  class="ladda-button btn btn-primary" > 确定
        <script>
            ...
            function cancel() {
                var seckill_id=$('#app').attr('data-seckill_id');
                if(!seckill_id){
            ...
            function sendIds() {
                ...
                var data = {
                    price : $('#seckill_id_'+seckill_id).data('price'),
                    title : $('#seckill_id_'+seckill_id).data('title'),
                    img : $('#seckill_id_'+seckill_id).data('img')
                ...
                obj.attr('data-seckill_id',seckill_id);
            ...
            $(document).ready(function () {
                ...
                $.get('{{route('admin.setting.micro.page.compoent.getSeckillsData',['seckill_id'=>request('seckill_id')])}}', data, function (ret) {
                    ...
                    success: function (result) {
                        ...
                        kkpager.generPageHtml({
                            ...
                            click:function(n){
                                ...
                                $.get("{{route('admin.setting.micro.page.compoent.getSeckillsData')}}?page="+n,data, function(data){
                                    $("#pagesList").html("");
                                    $("#pagesList").html(data);
                ...
                //分页
                var totalPage = "{{$seckills->lastPage()}}";
                var totalRecords = "1";
                var pageNo = getParameter('page');
                if (!pageNo) {
                    pageNo = 1;
