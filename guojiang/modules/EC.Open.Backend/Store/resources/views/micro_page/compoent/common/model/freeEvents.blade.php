...
@section('body')
    ...
    <div class="row">
        ...
        <form class="form-horizontal"  action="{{route('admin.setting.micro.page.compoent.getFreeEventsData')}}" method="get" id="search_spu_from">

            <div class="col-md-12">
                <div class="form-group">
                    <label for="exampleInputEmail1" class="col-sm-3 control-label"> 集call活动名称:</label>
                    <div class="col-sm-7">
                ...
                @else
                    @if(request('free_event_id'))
                        <div class="form-group">
                            <label for="exampleInputEmail1" class="col-sm-3 control-label"></label>
                            <div class="col-sm-6">
                                <span class="label label-default">free_event_id：{{request('free_event_id')}};不存在</span>
                            </div>
                        </div>
                    @endif
        ...
        <div class="panel-body">
            ...
            <div class="table-responsive col-sm-9" id="pagesList" data-free_event_id="{{request('free_event_id')}}">
...
@section('footer')
    ...
    <button type="button" onclick="sendIds();"  class="ladda-button btn btn-primary" > 确定
        <script>
            ...
            function cancel() {
                var free_event_id=$('#app').attr('data-free_event_id');
                if(!free_event_id){
            ...
            function sendIds() {
                ...
                console.log(free_event_id)


                var data = {
                    per_count : $('#free_event_id_'+free_event_id).data('per_count'),
                    title : $('#free_event_id_'+free_event_id).data('title'),
                    img : $('#free_event_id_'+free_event_id).data('img')

                };

                compoent_componet_html=compoent_html.replace('{#per_count#}',data.per_count);
                ...
                obj.attr('data-free_event_id',free_event_id);
            ...
            $(document).ready(function () {
                ...
                $.get('{{route('admin.setting.micro.page.compoent.getFreeEventsData',['free_event_id'=>request('free_event_id')])}}', data, function (ret) {
                    ...
                    success: function (result) {
                        ...
                        kkpager.generPageHtml({
                            ...
                            click:function(n){
                                ...
                                $.get("{{route('admin.setting.micro.page.compoent.getFreeEventsData')}}?page="+n,data, function(data){
                                    $("#pagesList").html("");
                                    $("#pagesList").html(data);
                ...
                //分页
                var totalPage = "{{$freeEvents->lastPage()}}";
                var totalRecords = "1";
                var pageNo = getParameter('page');
                if (!pageNo) {
                    pageNo = 1;
