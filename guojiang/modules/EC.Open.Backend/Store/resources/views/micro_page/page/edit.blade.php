    ...
    <div class="tab-content">
        ...
        <div class="tab-pane active">
            ...
            <div class="panel-body" style="margin-bottom:20px;">
                ...
                <a class="col-sm-12" style="margin-bottom:20px;">详细使用说明
                ...
                <div class="col-sm-12">
                    ...
                    <div class="col-sm-5 box-1 box-line">
                        ...
                        <div style="margin-top:20px;">
                            ...
                            <input type="text" class="form-control taginput" name="background_color" value="#FFFFFF"
                                   value=""/>

                            @else

                                <input type="text" class="form-control taginput" name="background_color"
                                       value="{{isset(json_decode($page->meta)->background_color)?json_decode($page->meta)->background_color:''}}"
                                       value=""/>
                            @endif
...
<script>
    ...
    @if($adverts->count())
            ...
            @foreach($adverts as $item)
                ...
                @if($item->advert_id>0)
                    ...
                    @elseif(stristr($item->advert->type,'componet_goods_group'))
                    ...
                    @elseif(stristr($item->advert->type,'componet_article'))

                    add('module-box',module_box_article_html);


                    @elseif($item->advert->type=='micro_page_componet_guess_like')

                    add('module-box',module_box_guess_like_html);

                    @endif

                    slide("{{$item->advert_id}}","{{$item->id}}")

                    meta("{{$item->meta}}","input_{{$item->id}}")
    ...
    function slide(id,d){
        ...
        li.find('.meta').addClass("input_"+d);
    }

    function meta(text,id){
        var meta=null;
         if(text!=''){
             var meta=JSON.parse(quotSpace(text));
         }
         if(meta){
             for (key in meta){
                 var obj=$('.'+id).find('input[name='+key+']');
                 obj.val(meta[key]);
             }
         }
     }
    ...
    function quotSpace(str) {
        var arrEntities = {'quot' : '"'};
    ...
    function save() {
        ...
        $('.module-box-slide').each(function (v, obj) {
            ...
            data[v] = {'sort': null, 'advert_id': null, 'micro_page_id': micro_page_id,'meta':{}}
            ...
            inputObj.each(function (k,obj) {

                var name=$(obj).attr('name');

                var value=$(obj).val();

                data[v]['meta'][name]=$(obj).val()
