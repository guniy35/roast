<div class="tabs-container">
    ...
    <ul class="nav nav-tabs">
        ...
        <li class=""><a no-pjax href="{{route('admin.promotion.gift.birthday.user')}}" aria-expanded="true">
                即将生日用户列表</a></li>
    </ul>
                            ...
                            <tr>
                                        ...
                                        @if(isset($item->gift)&&count($item->gift)>0)
                                            @foreach($item->gift as $ck=>$citem)
                                                @if(isset($citem->coupon->id))
                                                    <a href="{{route('admin.promotion.coupon.edit',$citem->coupon->id)}}"
                                                       target="_blank">
                                                        {{$citem->coupon->title}}
                                                        @if(count($coupons)<=0)
                                                            (已过期)
                                ...
                                <td>
                                    <a
                                            ...
                                            href="{{route('admin.promotion.gift.birthday.edit',['id'=>$item->id])}}">
                                        <i data-toggle="tooltip" data-placement="top"
                                           class="fa fa-pencil-square-o"
                                           title="编辑"></i></a>

                                    <span class="btn btn-xs btn-danger del-gift-birthday"
                                          data-url="{{route('admin.promotion.gift.birthday.delete',['id'=>$item->id])}}">
                                ...
                                <td>
                                    <a>
                                        <i class="fa switch @if($item->status) fa-toggle-on @else fa-toggle-off @endif"
                                           title="切换状态" value={{$item->status}}  data-date="{{$item->starts_at}}">
...
<script>
	$(function () {
		$('.del-gift-birthday').on('click', function () {
			var obj = $(this);
			swal({
				title: "确定删除该生日礼活动吗？",
				text: "",
	...
	$('.switch').on('click', function () {
		...
		console.log($(this));
		$.post("{{route('admin.promotion.gift.birthday.toggleStatus')}}",
			{
				status: value,
				aid: modelId,
				date: date
