...
<div class="tabs-container">
    <ul class="nav nav-tabs">
        <li class="{{ Active::query('status','all') }}"><a no-pjax
                ...
                <span class="badge">{{\GuoJiangClub\EC\Open\Backend\Store\Model\Order::whereBetween('status',[1,6])->where('type',5)->count()}}</span></a>
        </li>

        <li class="{{ Active::query('status',2) }}"><a no-pjax
                                                       href="{{route('admin.point-mall.orders.index',['status'=>2])}}">待发货
                <span class="badge">{{\GuoJiangClub\EC\Open\Backend\Store\Model\Order::where('status',2)->where('type',5)->count()}}</span></a>
        </li>
        <li class="{{ Active::query('status',3) }}"><a no-pjax
                                                       href="{{route('admin.point-mall.orders.index',['status'=>3])}}">待收货
                <span class="badge">{{\GuoJiangClub\EC\Open\Backend\Store\Model\Order::where('status',3)->where('type',5)->count()}}</span></a>
        </li>
        <li class="{{ Active::query('status',4) }}"><a no-pjax
                                                       href="{{route('admin.point-mall.orders.index',['status'=>4])}}">已收货待评价
                <span class="badge">{{\GuoJiangClub\EC\Open\Backend\Store\Model\Order::where('status',4)->where('type',5)->count()}}</span></a>
        </li>
        <li class="{{ Active::query('status',5) }}"><a no-pjax
                                                       href="{{route('admin.point-mall.orders.index',['status'=>5])}}">已完成
                <span class="badge">{{\GuoJiangClub\EC\Open\Backend\Store\Model\Order::where('status',5)->where('type',5)->count()}}</span></a>
        </li>
        <li class="{{ Active::query('status',6) }}"><a no-pjax
                                                       href="{{route('admin.point-mall.orders.index',['status'=>6])}}">已取消
                <span class="badge">{{\GuoJiangClub\EC\Open\Backend\Store\Model\Order::where('status',6)->where('type',5)->count()}}</span></a>
        </li>
        <li class="{{ Active::query('status',9) }}"><a no-pjax
                                                       href="{{route('admin.point-mall.orders.index',['status'=>9])}}">已删除
                <span class="badge">{{\GuoJiangClub\EC\Open\Backend\Store\Model\Order::where('status',9)->where('type',5)->count()}}</span></a>
        </li>
        ...
        <div id="tab-1" class="tab-pane active">
            ...
            <div class="panel-body">
                ...
                <div class="table-responsive">
                    <div id="orders">
                        @include('store-backend::point_mall.orders.includes.orders_list')
                    </div>
                </div>
...
@include('store-backend::point_mall.orders.includes.script')