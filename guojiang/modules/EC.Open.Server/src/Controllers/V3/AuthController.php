...
use GuoJiangClub\Server\Exception\UserExistsException;
use GuoJiangClub\Wechat\Server\Wx\WXBizDataCrypt;
...
class AuthController extends Controller
{
    ...
    public function openPlatform()
    {
        ...
        if (empty($userBind)) {
            $this->bind->create(['open_id' => $openId, 'type' => $type, 'user_id' => request()->user()->id]);
    ...
    private function bindWxInfo($openID, $type, $user_id)
    {
        ...
        if ($type == 'wechat') {
            $wxInfo = wechat_channel()->getUserInfo($openID,false,false);
