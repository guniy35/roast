...
class AuthController extends Controller
{
	...
	public function appSmsLogin()
	{
		$mobile  = request('mobile');
		$open_id = request('open_id');
		$code    = request('code');

		if (!$mobile || !$code || !$open_id) {
		...
		$token    = $user->createToken($mobile)->accessToken;
		$userBind = $this->userBindRepository->findWhere(['open_id' => $open_id])->first();
		if ($userBind && !$userBind->user_id) {
			$userBind->user_id = $user->id;
			$userBind->save();
		}

		if ($userBind && $userBind->unionid) {
			$binds = $this->userBindRepository->findWhere(['unionid' => $userBind]);
			if ($binds->count() > 0) {
